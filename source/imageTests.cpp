...
TEST_CASE("NaiveImageFilter") {
  ...
  cv::Mat kernel = cv::getStructuringElement(
      cv::MORPH_RECT, cv::Size(imageInfo.maskSize, imageInfo.maskSize));

  SECTION("MIN") {
    cv::erode(testImage, testImage, kernel);
    SECTION("Single thread") {
      NaiveImageFilter naiveImageFilter(1, AlgorithmType::MIN,
                                        imageInfo.maskSize);
	  ...
	  fileManager.saveImageToFile(image, std::string(ROOT_DIR) +
                                             "/output/imageTestsNaive.png");
    }

    SECTION("Multiple threads") {
      NaiveImageFilter naiveImageFilter(imageInfo.threadsNum,
                                        AlgorithmType::MIN, imageInfo.maskSize);
      naiveImageFilter.apply(image);
      const auto imagesAreTheSameSize = image.size() == testImage.size();
      cv::Mat check;
      cv::bitwise_xor(image, testImage, check);
  ...
  SECTION("MAX") {
    ...
    SECTION("Single thread") {
      NaiveImageFilter naiveImageFilter(1, AlgorithmType::MAX,
                                        imageInfo.maskSize);
      naiveImageFilter.apply(image);
      const auto imagesAreTheSameSize = image.size() == testImage.size();
      cv::Mat check;
    ...
    SECTION("Multiple threads") {
      NaiveImageFilter naiveImageFilter(imageInfo.threadsNum,
                                        AlgorithmType::MAX, imageInfo.maskSize);
      naiveImageFilter.apply(image);
      const auto imagesAreTheSameSize = image.size() == testImage.size();
      cv::Mat check;
      cv::bitwise_xor(image, testImage, check);
...
TEST_CASE("SmartImageFilter") {
  ...
  SECTION("MIN") {
    cv::erode(testImage, testImage, kernel);
    SECTION("Single thread") {
      SmartImageFilter smartImageFilter(1, AlgorithmType::MIN,
                                        imageInfo.maskSize);
      ...
      fileManager.saveImageToFile(image, std::string(ROOT_DIR) +
                                             "/output/imageTestsSmart.png");
      fileManager.saveImageToFile(testImage, std::string(ROOT_DIR) +
                                             "/output/imageTestsControl.png");
    }

    SECTION("Multiple threads") {
      SmartImageFilter smartImageFilter(imageInfo.threadsNum,
                                        AlgorithmType::MIN, imageInfo.maskSize);
      smartImageFilter.apply(image);
      const auto imagesAreTheSameSize = image.size() == testImage.size();
      cv::Mat check;
  ...
  SECTION("MAX") {
    cv::dilate(testImage, testImage, kernel);
    SECTION("Single thread") {
      SmartImageFilter smartImageFilter(1, AlgorithmType::MAX,
                                        imageInfo.maskSize);
      smartImageFilter.apply(image);
      const auto imagesAreTheSameSize = image.size() == testImage.size();
      cv::Mat check;
