  ...
  for (int i = 0; i < lhs.size().width; ++i) {
    if (lhs.at<uchar>(0, i) != rhs.at<uchar>(0, i)) {
      std::cout << "LHS vs RHS diff at: " << i << ".\n"
                << static_cast<int>(lhs.at<uchar>(0, i)) << " vs. "
                << static_cast<int>(rhs.at<uchar>(0, i)) << "\n\n";
    }
  }
}
...
TEST_CASE("NaiveSignalFilter") {
  ...
  cv::Mat kernel = cv::getStructuringElement(cv::MORPH_RECT,
                                             cv::Size(signalInfo.maskSize, 1));

  SECTION("MIN") {
    cv::erode(signalTestMat, signalTestMat, kernel);
    SECTION("Single thread") {
      NaiveSignalFilter naiveSignalFilter(1, AlgorithmType::MIN,
                                          signalInfo.maskSize);
      naiveSignalFilter.apply(signal);
      cv::Mat signalMat(1, signal.size(), CV_8UC1);
      copySignalToMat(signal, signalMat);
      const auto signalsAreTheSameSize =
          signalMat.size() == signalTestMat.size();
      cv::Mat check;
    ...
    SECTION("Multiple threads") {
      NaiveSignalFilter naiveSignalFilter(
          signalInfo.threadsNum, AlgorithmType::MIN, signalInfo.maskSize);
      naiveSignalFilter.apply(signal);
      cv::Mat signalMat(1, signal.size(), CV_8UC1);
      copySignalToMat(signal, signalMat);
      const auto signalsAreTheSameSize =
  ...
  SECTION("MAX") {
    ...
    SECTION("Single thread") {
      NaiveSignalFilter naiveImageFilter(1, AlgorithmType::MAX,
                                         signalInfo.maskSize);
    ...
    SECTION("Multiple threads") {
      NaiveSignalFilter naiveImageFilter(
          signalInfo.threadsNum, AlgorithmType::MAX, signalInfo.maskSize);
      naiveImageFilter.apply(signal);
      cv::Mat signalMat(1, signal.size(), CV_8UC1);
      copySignalToMat(signal, signalMat);
      const auto signalsAreTheSameSize =
...
TEST_CASE("SmartSignalFilter") {
  ...
  SECTION("MIN") {
    cv::erode(signalTestMat, signalTestMat, kernel);
    SECTION("Single thread") {
      SmartSignalFilter smartSignalFilter(1, AlgorithmType::MIN,
                                          signalInfo.maskSize);
    ...
    SECTION("Multiple threads") {
      SmartSignalFilter smartSignalFilter(
          signalInfo.threadsNum, AlgorithmType::MIN, signalInfo.maskSize);
      smartSignalFilter.apply(signal);
      cv::Mat signalMat(1, signal.size(), CV_8UC1);
      copySignalToMat(signal, signalMat);
  ...
  SECTION("MAX") {
    cv::dilate(signalTestMat, signalTestMat, kernel);
    SECTION("Single thread") {
      SmartSignalFilter smartSignalFilter(1, AlgorithmType::MAX,
                                          signalInfo.maskSize);
      smartSignalFilter.apply(signal);
      cv::Mat signalMat(1, signal.size(), CV_8UC1);
      copySignalToMat(signal, signalMat);
