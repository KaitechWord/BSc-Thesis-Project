...
static const auto minSingleTextFile =
    std::string(ROOT_DIR) + "/output/SignalNaiveMinSingle.txt";
static const auto minMultiTextFile =
    std::string(ROOT_DIR) + "/output/SignalNaiveMinMulti.txt";
static const auto maxSingleTextFile =
    std::string(ROOT_DIR) + "/output/SignalNaiveMaxSingle.txt";
static const auto maxMultiTextFile =
    std::string(ROOT_DIR) + "/output/SignalNaiveMaxMulti.txt";

NaiveSignalFilter::NaiveSignalFilter(int threadNum, AlgorithmType algType,
                                     int maskSize)
...
void NaiveSignalFilter::apply(std::vector<uint8_t> &signal) {
  ...
  if (threadsNum > size) {
    std::cout << "The number of threads is bigger than the size of the signal. "
                 "Setting number of threads to size.\n";
    threadsNum = size;
  }
  int sizeOfOneThread = size / threadsNum;
  int remainder = size % threadsNum;
  std::vector<std::vector<uint8_t>> newSignals(threadsNum,
  ...
  for (int i = 0; i < threadsNum; ++i) {
    ...
    if (firstIndex < maskOneHalfLength &&
        lastIndex > size - maskOneHalfLength - 1) {
      part = SignalPart::Full;
    } else if (firstIndex < maskOneHalfLength) {
      part = SignalPart::Left;
    } else if (lastIndex > size - maskOneHalfLength - 1) {
      part = SignalPart::Right;
    } else {
      part = SignalPart::Middle;
    }
    threads.emplace_back(std::thread(&NaiveSignalFilter::filter, this,
                                     std::ref(newSignals[i]), firstIndex,
                                     lastIndex, part));
  }
  ...
  std::cout << "Naive "
            << (this->algType == AlgorithmType::MIN ? "min." : "max.")
            << " signal "
            << (threadsNum == 1 ? "(single-threaded)" : "(multi-threaded)")
            << " filter execution time : " << execTime << "ms.\n";
  std::cout << std::fixed
            << "Elem/sec:" << this->data.size() / (execTime * 0.001)
            << std::endl;
  const auto textFile =
      this->algType == AlgorithmType::MIN
          ? (threadsNum == 1 ? minSingleTextFile : minMultiTextFile)
          : (threadsNum == 1 ? maxSingleTextFile : maxMultiTextFile);
...
void NaiveSignalFilter::filter(std::vector<uint8_t> &newSignal, int firstIndex,
                               int lastIndex, SignalPart part) {
  int signalSize = this->data.size();
  // By one half I mean the half without the center point, e.g. maskSize = 5,
  // thus the half length is 2
  int maskOneHalfLength = this->maskSize / 2;
  // Setting the initial value of index of MIN/MAX value that we are looking for
  // to one 	behind the start of mask
  int indexOfTargetValue =
