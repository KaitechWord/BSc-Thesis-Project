...
void SmartImageFilter::filter(cv::Mat& newImage, int firstIndex, int lastIndex) {
	...
	for (; i <= lastIndex; ) {
		...
		auto secondMaskCentreIndex = std::clamp(colIndex + maskSize, secondMaskFarLeftIndex, secondMaskFarRightIndex);
		...
		if (i == lastIndex)
			break;
		if(secondMaskCentreIndex == cols - 1){
			colIndex = 0;
			i = rowIndex * cols;  // Move to the first column of the new row
			// std::cout << "i before next row: " << i << std::endl;
			// std::cout << "rowindex for next row: " << rowIndex + 1 << std::endl;
			// rowIndex++;
			if(rowIndex++ == canBreak){
				break;
			}
		}
		else {
	...
	// std::cout << "actual colindex: " << colIndex << std::endl;
	// std::cout << "actual rowindex: " << rowIndex << std::endl;
	...
	for (; i <= lastIndex; ) {
		...
		const auto modFarTopIndex = farTopIndex % maskSize;
		auto modFarTopIndexCopy = modFarTopIndex;
		for (auto j = farBotIndex; j == farBotIndex; ++j) {
			//pozbyc sie modulo na rzecz ifa (++ z poprzedniej wartosci j)
			// prefixesPostfixes[getIndex(firstMaskFarRightIndex, 0, modFarTopIndexCopy, cols, maskSize)] = this->data.at<uchar>(j, firstMaskFarRightIndex);
			auto flattenColIndexStart = firstMaskFarRightIndex * doubleMaskSize;
			auto flattenColIndexEnd = 0;
