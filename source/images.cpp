...
int main(int argc, char *argv[]) {
  ...
  if (argc == 1) {
    ...
    imageInfo.variant =
        std::stoi(argv[2]) == static_cast<int>(AlgorithmType::MIN)
            ? AlgorithmType::MIN
            : AlgorithmType::MAX;
    imageInfo.approach =
        std::stoi(argv[3]) == static_cast<int>(FilterApproach::NAIVE)
            ? FilterApproach::NAIVE
            : FilterApproach::SMART;
    imageInfo.dataPath = argv[4];
    imageInfo.maskSize = std::stoi(argv[5]);
  } else {
    std::cerr << "Invalid number of arguments! The argumentas are: "
                 "<threads_num> <variant> <approach> <file_path> <mask_size>";
    return 1;
  }
  ...
  std::unique_ptr<ImageFilter> imageFilter =
      imageInfo.approach == FilterApproach::NAIVE
          ? std::unique_ptr<ImageFilter>(std::make_unique<NaiveImageFilter>(
                imageInfo.threadsNum, imageInfo.variant, imageInfo.maskSize))
          : std::unique_ptr<ImageFilter>(std::make_unique<SmartImageFilter>(
                imageInfo.threadsNum, imageInfo.variant, imageInfo.maskSize));

  std::cout << "Size:" << image.size();
  std::cout << " Threads:" << imageInfo.threadsNum;
  std::cout << "Mask:" << imageInfo.maskSize << std::endl;
  imageFilter->apply(image);
  fileManager.saveImageToFile(image, std::string(ROOT_DIR) +
                                         "/output/imageResult.png");
