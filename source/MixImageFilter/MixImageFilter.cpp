...
MixImageFilter::MixImageFilter(int threadNum, AlgorithmType algType, int maskSize)
	: ImageFilter(threadNum, algType, maskSize)
{}

void MixImageFilter::apply(cv::Mat& image) {
	this->data = image;
	...
	for (auto i = 0; i < threadsNum; ++i) {
		...
		threads.emplace_back(std::thread(&MixImageFilter::filter, this, std::ref(newImage), firstIndex, lastIndex));
	}
	...
	std::cout << "Mix " << (this->algType == AlgorithmType::MIN ? "min." : "max.") << " image " << (threadsNum == 1 ? "(single-threaded)" : "(multi-threaded)") << " MaskSize: " << this->maskSize << " filter execution time : " << execTime << "ms.\n Elem./sec.: " << (this->data.cols * this->data.rows / std::chrono::duration<double, std::milli>(end - start).count()) * 0.001 << "\n";
...
int MixImageFilter::getIndexPrefix( const Indices& indices, int left, int right, int row ){
...
int MixImageFilter::getIndexSuffix( const Indices& indices, int left, int right, int row ){
	auto relativeLeft = left;
	auto relativeRight = right;
	auto relativeRow = row - indices.top;
	return this->maskSize * this->maskSize * relativeLeft + this->maskSize * relativeRight + relativeRow;
...
void MixImageFilter::calculatePrefixes( const Indices& indices, Precalculation& precalculation, int row )
{
...
void MixImageFilter::calculateSuffixes( const Indices& indices, Precalculation& precalculation, int row )
{
...
void MixImageFilter::precalculate( const Indices& indices, Precalculations& precalculations )
{
...
void MixImageFilter::calculateFreshPrefixes( const Indices& indices, Precalculation& precalculation, int row )
{
	auto prefixRightIndex = indices.firstMask.right;
...
void MixImageFilter::calculateFreshSuffixes( const Indices& indices, Precalculation& precalculation, int row )
{
	auto suffixLeftIndex = indices.secondMask.left; 
...
void MixImageFilter::precalculateFreshRow( const Indices& indices, Precalculations& precalculations )
{
...
void MixImageFilter::setPrefixOnlyMaskExtremum( cv::Mat& newImage, Indices& indices, Precalculation& precalculation )
{
...
void MixImageFilter::setAffixMixMaskExtrema( cv::Mat& newImage, Indices& indices, Precalculations& precalculations )
{
...
void MixImageFilter::setSuffixOnlyMaskExtremum( cv::Mat& newImage, Indices& indices, Precalculation& precalculation )
{
...
void MixImageFilter::setExtrema( cv::Mat& newImage, Indices& indices, Precalculations& precalculations )
{
...
int MixImageFilter::getIndexPrefixFromRowToBeOverriden(const Indices& indices, int left, int right){
	auto relativeLeft = left;
	auto relativeRight = right;
	return this->maskSize * this->maskSize * relativeLeft + this->maskSize * relativeRight + indices.rowToBeOverriden;
}

int MixImageFilter::getIndexSuffixFromRowToBeOverriden(const Indices& indices, int left, int right){
	auto relativeLeft = left;
	auto relativeRight = right;
	return this->maskSize * this->maskSize * relativeLeft + this->maskSize * relativeRight + indices.rowToBeOverriden;
}

int MixImageFilter::getIndexPrefixFresh(const Indices& indices, int left, int right, int row){
	auto relativeLeft = left;
	auto relativeRight = right;
	auto relativeRow = row - indices.top;
...
int MixImageFilter::getIndexSuffixFresh(const Indices& indices, int left, int right, int row){
	auto relativeLeft = left;
	auto relativeRight = right;
	auto relativeRow = row - indices.top;
	auto relativeRowWithOverridenOffset = relativeRow + indices.rowToBeOverriden + 1;
...
void MixImageFilter::setFreshPrefixOnlyMaskExtremum( cv::Mat& newImage, Indices& indices, Precalculation& precalculation )
{
...
void MixImageFilter::setFreshAffixMixMaskExtrema( cv::Mat& newImage, Indices& indices, Precalculations& precalculations )
{
	auto maskOneHalfLength = static_cast<int>( std::floor(this->maskSize / 2) );
...
void MixImageFilter::setFreshSuffixOnlyMaskExtremum( cv::Mat& newImage, Indices& indices, Precalculation& precalculation )
{
...
void MixImageFilter::setFreshExtrema( cv::Mat& newImage, Indices& indices, Precalculations& precalculations )
{
...
void MixImageFilter::updateRowColumnAndIndex( int& row, int& column, int& index )
{
...
void MixImageFilter::updateRowColumnAndIndices( int& row, int& column, int& index, Indices& indices )
{
	auto valueToIterate = this->maskSize + 1;
	column += valueToIterate;  // Move to next mask position
...
void MixImageFilter::updateIndices( Indices& indices, int row, int column ){
	auto maskOneHalfLength = static_cast<int>(std::floor(this->maskSize / 2));
...
void MixImageFilter::filter(cv::Mat& newImage, int firstIndex, int lastIndex) {
