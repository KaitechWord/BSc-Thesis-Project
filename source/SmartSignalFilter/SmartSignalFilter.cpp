    ...
    if (partOfSignalType == PartOfSignal::WHOLE) {
        ...
        for (int startOfPrefixMask = 0; startOfPrefixMask < signalSize; startOfPrefixMask += maskSize + 1) {
            ...
            if (endOfPrefixMask >= signalSize) {
                ...
                // one full mask at this point
                //zle to jest(usuniete), trzeba to rozwiazac tak jak naive, ale biorac pod uwage to, ze startOfPrefixMask to nie jest srodek maski, a poczatek,
                //wiec tak naprawde srodkiem bedzie startOfPrefixMask + maskOneHalfLength
